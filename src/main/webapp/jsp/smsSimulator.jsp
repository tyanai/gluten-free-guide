...
<link rel="stylesheet" media="all" type="text/css" href="../include/balloons.css" />
...
%>
<TD  colspan="1"  style="width: 50%" ><input type="text" value="<%=tmpprdsel%>" id="productSelect3" name="productSelect3" ><script>
		$(function() { 
			$("#productSelect3").autocomplete
        ({ 
...
<TD  colspan="1"  style="width: 50%" ><input type="text" value="<%=tmpcmpsel%>" id="companySelect3" name="companySelect3" ><script>
		$(function() { 
			$("#companySelect3").autocomplete
        ({ 
            source: function( request, response )
            {                      
                $.ajax(
                { 
                    url: "getCompanyData.jsp",
                    data: {
                            term: request.term, 
                          },        
                    type: "POST",  
        ...
        <font face="Verdana, Arial, Helvetica, sans-serif">
          ...
          <select name="company" class="textfieldHeb" onchange="processSelectionCompany();">
          	...
          	java.util.Iterator<String> itr = null;
						...
						if ((""+tmpCompanyCounter).equals(""+theCompany)) {
							companySelect = tmpCompany;
							tmpForCompanyName = tmpCompany;
						}
...
<dt>אזהרה!</dt>
<dd>
<p>רשימה זו מכילה את כל מוצרי "<%=tmpForCompanyName%>" הקיימים במדריך.</p>
<p>מכיוון שכך, יש לבחור מוצר ספציפי מהרשימה הנ"ל בכדי לדעת האם הוא נטול גלוטן.</p>
<em></em>
</dd>
</dl>
</td></tr></table>
