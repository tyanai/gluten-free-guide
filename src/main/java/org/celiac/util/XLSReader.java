...
public class XLSReader {
	...
	public Map<String, CompanyTable> readWorkBook(POIFSFileSystem fs) throws Exception {
		...
		try {
			...
			while (it.hasNext()) {
				...
				if (sheetName.indexOf(".") > -1)throw new Exception(
	...
	public GFUser[] readUsersWorkBook(InputStream stream) throws Exception {
		POIFSFileSystem fs = new POIFSFileSystem(stream);
		return readUsersWorkBook(fs);
	}

	public GFUser[] readUsersWorkBook() throws Exception {
		POIFSFileSystem fs = new POIFSFileSystem(this.getClass().getResourceAsStream(usersInputFile));
		return readUsersWorkBook(fs);
	}

	public GFUser[] readUsersWorkBook(POIFSFileSystem fs) throws Exception {

		GFUser[] usersTable = null;

		try {
			// POIFSFileSystem fs = new
			// POIFSFileSystem(this.getClass().getResourceAsStream(usersInputFile));
			HSSFWorkbook wb = new HSSFWorkbook(fs);
			...
			for (; j < howManyRows; j++) {
				...
				try {
					try {
						...
						usersTable[j].setCELIAC_MEMBER_ID(new String(tmp));
					} catch (Exception e) {
						usersTable[j].setCELIAC_MEMBER_ID(new String(row.getCell((short) 0).getRichStringCellValue().getString()));
					}
					try {
						usersTable[j].setUSER_TZ(new String(row.getCell((short) 1).getRichStringCellValue().getString()));
					} catch (Exception e) {
						String tmp = "" + row.getCell((short) 1).getNumericCellValue();
						if (tmp.indexOf(".") > 0)
							tmp = tmp.substring(0, tmp.indexOf("."));
						usersTable[j].setUSER_TZ(new String(tmp));
					}
					usersTable[j].setEXPIRATION_DATE(row.getCell((short) 2).getDateCellValue());
					usersTable[j].setDID_BUY_THE_BOOK(new String("" + row.getCell((short) 3).getBooleanCellValue()));
				} catch (Exception e) {
	...
	private GFUser[] getAdminUser() throws Exception {
		...
		GFUser[] gfAdminUser = new GFUser[2];
